...
&& in_array($extension, $allowedExts)) 
{
  ...
  else 
  {
    ...
    {
      header("Location:library_student.php?msg=Already exists.&st_id=$_REQUEST[st_id]");
    } 
    else {
      move_uploaded_file($_FILES["upload"]["tmp_name"],"library/temp/videos/" . $_FILES["upload"]["name"]);
      header("Location:library_student.php?msg=Uploaded Successfully Sent To ADMIN for Confirmation.&st_id=$_REQUEST[st_id]");
    }
  }
}
 else {
  header("Location:library_student.php?msg=Invalid File.&st_id=$_REQUEST[st_id]");
}
}
