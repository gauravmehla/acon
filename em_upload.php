...
&& in_array($extension, $allowedExts)) 
{
  ...
  else 
  {
    ...
    {
      header("Location:library_teacher.php?msg=Already exists.&em_id=$_REQUEST[em_id]");
    } 
    else {
      move_uploaded_file($_FILES["upload"]["tmp_name"],"library/temp/videos/" . $_FILES["upload"]["name"]);
      header("Location:library_teacher.php?msg=Uploaded Successfully Sent To ADMIN for Confirmation.&em_id=$_REQUEST[em_id]");
    }
  }
}
 else {
  header("Location:library_teacher.php?msg=Invalid File.&em_id=$_REQUEST[em_id]");
}
}
